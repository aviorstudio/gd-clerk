...
v0.1.0 must not be published until a live email-code run has been exercised. That proof is a successful `e2e` workflow artifact for the same commit and ZIP, not a file committed to the repository. `docs/E2E_ACCEPTANCE.md` is rejected if it is added. The evidence must record the tested ZIP sha256, the pinned clerk bundle sha256, an allowed origin, an https Frontend API origin, a delivered sign-in code, a confirmed sign-out, a signed-out reload, an observed network failure, and an observed Godot web export. Sign-up is delivered only when Smart protection does not present an interactive challenge. A presented Turnstile challenge is recorded as unsolved and is not acceptance. None of those observations exist yet.
