...
When the loaded environment has Smart bot protection enabled, the addon mounts an unhidden `<div id="clerk-captcha">` before `signUp.create()` if the consumer has not supplied a visible one. If that element is missing, Clerk's own bundle falls back to an invisible widget. This addon treats that fallback as a failure and does not continue sign-up. It does not mount a slot when the loaded environment explicitly reports captcha off. A live Cloudflare interactive challenge has not been executed against a configured Clerk instance.
