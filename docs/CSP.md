...
Godot's `JavaScriptBridge.eval` fallback needs `script-src 'unsafe-eval'`. The export plugin avoids that fallback by injecting same-origin `<script src="gd-clerk/clerk.browser.js">` and `<script src="gd-clerk/gd_clerk_bridge.js">` before `</head>`. Do not load `@clerk/clerk-js` from a floating CDN URL.
